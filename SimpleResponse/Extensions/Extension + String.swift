...
extension String {
    ...
    func formatDateAndTime() -> (String, String) {
        let dateAndTime = self.components(separatedBy: ", ")
        let date = dateAndTime[0].toDate(.isoDate)!
        ...
        let monthInName = DateFormatter().monthSymbols[month - 1]
        let year = Calendar.current.dateComponents([.day, .year, .month], from: date).year!
        
        let time = dateAndTime[1].prefix(4)
        
        return (("\(day) \(monthInName) \(year)"), (String(time)))
    }
